...
function calculateNote(event) {
    ...
    resultNote.textContent = `Buenos días ${name}, su nota es de: ${finalNote.toFixed(2)}`;

    if (finalNote < 3.5) {
        resultNote.textContent = `Buenos días ${name}, ha perdido la materia.`;
        resultNote.style.color = "black";
    } else if (finalNote >= 3.5 && finalNote <= 4.5) {
        resultNote.textContent = `Buenos días ${name}, ha ganado la materia.`;
        resultNote.style.color = "orange";
    } else {
        resultNote.textContent = `Buenos días ${name}, su nota es sobresaliente.`;
